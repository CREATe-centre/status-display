...
function selectRecentRelated() {
	jQuery(function($) {
		getRelatedTweetsTree( $( "#select" ).val(), 1 );
	});
};
 ...
 *
 * Parameters: type - type of query to call. Currently, there are 5 type of
 * query: "user" - get user profile "timeline" - get user tweets "follower" -
 * get list of followers "retweets" - get list of retweets of a tweet "status" -
 * get information on a tweet id - tweet id username - user screen name user_id -
 * user id callback - callback function will be called after the data is
 * returned
 */
// Send request to getdata.php to get the required data, callback function will
// be called once there is a response.
 ...
 *
 * call to php file to get all related tweets after a specific time the returned
 * data is then used to populate an array to stored for future use the data is
 * then display on the website
 *
 * Parameters: option - choose which range of time to get the related tweets: 1 -
 * all the tweets 2 - 1 day 3 - 1 week 4 - 2 weeks 5 - 3 weeks 6 - 1 month page -
 * page number of the dataset that we will retrieve, only 10 main tweet per page
 *
 */
function getRelatedTweetsTree(option, page) {
	...
	jQuery(function($) {
		var analysestream = function(data) {
			...
			// Update navigation display, next page button, previous page
			// button, page number.
			updatePageNavigation();
 ...
 *
 * update the navigation display of the page to the newest state action
 * includes: -disable, enable navigation buttons -update page number
 *
 */
function updatePageNavigation() {
	jQuery(function($) {
		...
		if (totalPage > 0) {
			...
			document.getElementById( "p" ).innerHTML = currentPage + "/"
					+ totalPage;
		} else {
			prevBtn.prop( 'disabled', true );
			nextBtn.prop( 'disabled', true );
 ...
 *
 * recursively convert the json component of this object and its children into
 * Status
 *
 * Parameters: original - the original object with json component
 *
 * Returns: return the final converted object
 */
function convertJsonIntoStatusObj(original) {
	...
	if (original["children"]) {
		for (i in original["children"]) {
			obj["children"]
					.push( convertJsonIntoStatusObj( original["children"][i] ) );
		}
 ...
 *
 * Parameters: array - the array to search stack - used to track the location
 * String for each tweet
 *
 * Returns: return the html representation of the tweets array tree
 */
function recursiveList(array, stack) {
	...
	for (var i = 0; i < array.length; i++) {
		...
		// Change display of tweets that are not originally recorded in the
		// database.
		var classHtml = "";
		...
		if (array[i]["children"].length > 0) {
			result += "><div id=\"" + status.id
					+ "\" onClick=\"displayCurrentStatus(\'" + pos.toString()
					+ "\')\"><button " + classHtml + "><b>"
					+ status.user.screenName + ":</b> " + status.text
					+ "</div>"
					+ recursiveList( array[i]["children"], pos.toString() )
					+ "</button>";
		} else {
			result += "id=\"" + status.id
					+ "\" onClick=\"displayCurrentStatus(\'" + pos.toString()
 ...
 *
 * display information of a status, show any related information on the map and
 * the chart is called when an item is clicked from the related tweet list
 *
 * Parameters: locationString - indicates the status location in the array
 */
 ...
 *
 * retrieve the object in the array from locationString is called by many
 * functions
 *
 * Parameters: locationString - indicates the status location in the array
 *
 * Returns: return the object found from the location String
 */
 ...
 *
 * get status from location string then pass to another function to generate
 * html string containing information of the status
 *
 * Parameters: locationString - indicates the status location in the array
 *
 * Returns: the html string containing information of the status
 *
 * See also: <getObjFromString> <generateStatusInfoFromStatus>
 */
 ...
 *
 * Parameters: status - the status object to analyse
 *
 * Returns: the html string containing information of the status
 */
function generateStatusInfoFromStatus(status) {
	var response = "<ul><b>Poster info</b>";
	response += "<li>screen name: " + status.user.screenName + "</li><li>following: "
			+ status.user.friendsCount + "</li><li>follower: "
			+ status.user.followersCount
			+ "</li></ul><ul><b>Tweet info</b><li>tweet time: "
			+ status.createdAt + "</li>";
	if (status.location) {
		response += "<li>status location: " + status.location + "</li>";
	}
	if (status.coordinates) {
		response += "<li>status coordinates: " + status.coordinates[0] + ", "
				+ status.coordinates[1] + "</li>";
	}
 ...
 *
 * Find all status from the location String array then generate information on
 * all of them and put into a table
 *
 * Parameters: array - array of location String
 *
 * Returns: the html string containing information on all the requested statuses
 */
		...
		if (table[i]) {
			...
			for (var j = 0; j < table[i].length; j++) {
				...
				if (table[i][j].split( "," ).length == 1) {
					...
					originContent += "<tr><td rowspan=\"" + rowspan
							+ "\" valign=\"top\">"
							+ generateStatusInfoFromString( table[i][j] )
							+ "</td>";
					...
					for (var k = 0; k < all[i]["children"].length; k++) {
						...
						originContent += "<td>"
								+ generateStatusInfoFromStatus( all[i]["children"][k]["status"] )
								+ "</td></tr>";
					}
					...
					retweetsLength++;
					hasNewRetweets = true;
					retweetsContent += "<tr><td>"
							+ generateStatusInfoFromString( table[i][j] )
							+ "</td>";
					// If has not show parent tweet, show it one time then stop.
					if ( ! showOrigin) {
						retweetsContent += "<td rowspan=\"" + table[i].length
								+ "\" valign=\"top\">"
								+ generateStatusInfoFromString( i.toString() )
								+ "</td>";
						showOrigin = true;
					}
					retweetsContent += "</tr>";
